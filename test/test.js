...
describe('example programs', function () {
  ...
  it('01-async', async () => {
    expect((await p(execFile)('node', ['src/01-async.js'])).stdout.trim()).to.equal('4.302651')
  })

  it('02-async-function', async () => {
    expect((await p(execFile)('node', ['src/02-async-function.js'])).stdout.trim()).to.equal('4.302651')
  })

  it('03-async-function-err', async () => {
    const result = await p(execFile)('node', ['src/03-async-function-err.js']).catch(err => err)

    expect(result.stderr).to.include('getaddrinfo ENOTFOUND data.fixer.io-error')
  ...
  it('05-no-concurrency', async () => {
    expect((await p(execFile)('node', ['src/05-no-concurrency.js'])).stdout.trim()).to.equal('4.302651 1.157548')
  })

  it('06-concurrency', async () => {
    expect((await p(execFile)('node', ['src/06-concurrency.js'])).stdout.trim()).to.equal('4.302651 1.157548')
  })
  ...
  it('11-promise-all-for-two', async () => {
    expect((await p(execFile)('node', ['src/11-promise-all-for-two.js'])).stdout.trim()).to.equal('4.302651 1.157548')
  })

  it('12-promise-all-for-two-with-map', async () => {
    expect((await p(execFile)('node', ['src/12-promise-all-for-two-with-map.js'])).stdout.trim()).to.equal('4.302651 1.157548')
  })

  it('13-promise-race', async () => {
    expect((await p(execFile)('node', ['src/13-promise-race.js'])).stdout.trim()).to.equal('undefined 4.302651')
  })

  it('14-promise-then', async () => {
    expect((await p(execFile)('node', ['src/14-promise-then.js'])).stdout.trim()).to.equal('1')
  })
